...
def logger_error(file, module, message):
    relative_dir = os.path.dirname(__file__)
    today = date.today()
    curDT = datetime.now()
    file_time_stamp = today.strftime("%y-%m-%d")
    ...
    file_name = file_time_stamp + "_error.log"
    try:
        file1 = open("logs/" + file_name, "a")  # append mode
    except:
        print("logger.py", "logger_error", ("Didn't find output folder, checking relative path", relative_dir))
        file1 = open( relative_dir + "/logs/" + file_name, "a")
    print(log)
    file1.write(str(log) + "\n")
    file1.close()
